...
from mf import MFModel, MF, NMF, LightGCN, ItemCF, MFSVD, SVD, MFG, MFBE, MFBESVD, MFMBE, MFProfile
import random
...
parser.add_argument("--model",   "-m",  type = str, default = "mf", help = "mf, light, ..., mfs")
...
parser.add_argument("--out_dir", "-o",  type = int, default = 201)
...
parser.add_argument("--num_negative", "-g", type = int, default = 30)
    ...
    if ana==True:
        ...
        for i in range(len(user_sel)):
            for j in range(item_mean.shape[0]):
                iii.append((i, j))
                sig_ = (user_mean[i, :] ** 2 * user_std[i, :]).sum() + (item_mean[j, :] ** 2 * item_std[j, :]).sum() + (user_std[i, :] * item_std[j, :]).sum()
                sig.append(sig_)
...
def get_gpu_info():
    ...
    print("GPU({:s}): total = {:.2f}G, used = {:.2f}G, free = {:.2f}G".format(gpu_name, total_size, use_size, free_size))
...
class Dataset_CF(object):
    ...
    def get_negs(self, user):
        ...
        s = 0
        while s < self.num_negative:
            js = np.random.choice(self.num_item, int(1.2 * self.num_negative), p = self.prob, replace=False)
            #js = np.random.randint(0, self.num_item, int(1.2 * self.num_negative))
            for j in js:
                if ( (user, j) not in self.mat ) and (s < self.num_negative):
                    items_neg[s] = j
...
def get_mf_train(name="pinterest", batch_size=1024, num_negative=10, svd=False, sample_alpha=0, user_index = 'user_prof_grine_ind', item_index = 'Genere_ind'):
    # user_index = 'user_prof_coarse_ind', item_index = 'Genere_ind'
    ...
    train_mat = torch.sparse_coo_tensor(train_data[[user_index, item_index]].values.T, train_data['type'].values, (num_user, num_item), dtype = torch.float32)
    # pdb.set_trace()
    ...
    test_pos_item = zip(test_data[user_index].values, test_data[item_index].values)
...
def get_train_new(name, batch_size, num_neg=10, train_type="active", matrix_type="sym", svd=False, sample_alpha=0):
    num_user, num_item, train_loader, mat, train_mat, user_total, test_sample, svd_mat = get_mf_train(name, batch_size, num_neg)
...
print("dataset={}, learning_rate={}, batch_size={}, test_type={}, patience = {}".format(dataset, learning_rate, batch_size, test_type, patience))
print("num_negative={}, train_type={}, embedding_dim={}, matrix_type={}".format(num_negative, train_type, embedding_dim, matrix_type))
print("Start loading train data ...")
start_time = time.time()
num_user, num_item, train_loader, mat, train_mat, test_user_total, test_sample, svd_mat = get_train_new(dataset, batch_size, num_negative, train_type, matrix_type, svd, sample_alpha)
...
model.name = "topk_{}_{}_batch_{}_dim_{}_{}_neg_{}_{}_{}_norm_{}_emb_dropout{}_dnn_{}".format(args.topk, model.name, batch_size, embedding_dim, dataset, num_negative, optimizer_name, learning_rate, normed, emb_dropout, use_dnn)
...
if check_model_trainable(model): 
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", factor=0.75, patience=5, verbose=True, min_lr=0.000001)
